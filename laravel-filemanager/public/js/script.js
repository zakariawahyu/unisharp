...
$('#list-sort-asc').click(function() {
  sort_type = 'asc';
  loadItems();
});

$('#list-sort-desc').click(function() {
  sort_type = 'desc';
  loadItems();
});
...
function loadItems(page = '') {
  ...
  performLfmRequest('jsonitems', {show_list: show_list, sort_type: sort_type, search: search, page: page}, 'html')
    .done(function (data) {
      var response = JSON.parse(data);
      $('#content').html(response.html);
      $('#nav-buttons > ul').removeClass('hidden');
      $('#working_dir').val(response.working_dir);
      $('#current_dir').text('Current Directory : '+response.working_dir);
...
function useFile(file_url) {
  ...
  if (window.opener || window.tinyMCEPopup || field_name || getUrlParam('CKEditorCleanUpFuncNum') || is_ckeditor) {
    if (window.tinyMCEPopup) { // use TinyMCE > 3.0 integration method
      ...
      /* useTinymce4AndColorbox(url, field_name); */ /* FILE ASLI */
        useTinymce4AndColorbox(file_path, field_name);
